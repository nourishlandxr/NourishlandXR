...
function createSpatialPlant(projectId, siteId, placeId, data) {
    ...
    const now = new Date().toISOString();
    const plant = existingPlant || { id: plantId, commonName, scientificName, cultivar: '', family: String(data.family || ''), origin: '', plantType: '', layer: '', uses: [], propagation: [], summary, image: String(data.image || ''), imageAttribution: String(data.imageAttribution || ''), imageLicense: String(data.imageLicense || ''), source: String(data.source || ''), sourceId: String(data.sourceId || ''), sourceUrl: String(data.sourceUrl || ''), externalSources: Array.isArray(data.externalSources) ? data.externalSources : [], visibility, created: now, modified: now };
