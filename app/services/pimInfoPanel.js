...
export const INFO_HELP = 'Hold any honeycomb cell until it fills to read its information here. Parent cells also open their branches. Hide this companion while walking; select +INFO to bring it back. Edit information opens the same knowledge used in the desktop editor.';
...
export function infoPanelPose(matrix, heading = null) {
    ...
    return { right, center: { x: matrix[12] - right.x * .62 + right.z * .85,
...
export function createPimInfoPanel({ root, onEdit = () => {} } = {}) {
    ...
    element.className = 'nlxr-info-panel'; element.setAttribute('aria-label', 'Info companion');
    root?.append(element);
    ...
    const pages = () => infoPages(text());
    const controls = () => hidden ? ['+INFO'] : ['Hide', 'Help', 'Previous', 'Next', ...(selection ? ['Edit'] : [])];
    function act(action) {
        if (action === '+INFO') hidden = false;
    ...
    function render() {
        ...
        if (!hidden) {
            const label = document.createElement('small'); label.textContent = '+INFO · ' + (selection?.plant || 'YOUR COMPANION');
            ...
            const metadata = document.createElement('small'); metadata.textContent = help || !selection ? 'Hide any time. +INFO restores this panel.' : [selection.scope === 'specimen' ? 'Local specimen' : selection.scope === 'species' ? 'Species knowledge' : 'Scope unspecified', selection.status, selection.evidence].filter(Boolean).join(' · ');
        ...
        for (const action of controls()) {
            ...
            button.dataset.infoAction = action; button.setAttribute('aria-label', action === 'Edit' ? 'Edit information' : action === '+INFO' ? 'Restore Info panel' : action);
    ...
    function canvas(card) {
        ...
        gradient.addColorStop(0, 'rgba(103,145,104,.66)'); gradient.addColorStop(1, 'rgba(18,62,42,.67)');
        ctx.fillStyle = gradient; ctx.beginPath(); ctx.roundRect(4, 4, 792, c.height - 8, 26); ctx.fill();
        ctx.strokeStyle = 'rgba(210,246,191,.8)'; ctx.lineWidth = 2; ctx.stroke();
        ctx.textBaseline = 'top'; ctx.fillStyle = '#f2fae9';
        if (!card.hidden) {
            ...
            ctx.font = '400 27px system-ui'; card.lines.forEach((line, i) => ctx.fillText(line, 34, 208 + i * 37, 732));
        ...
        actions.forEach((action, i) => {
            ...
            ctx.fillStyle = disabled ? 'rgba(211,241,202,.08)' : 'rgba(211,241,202,.2)'; ctx.fillRect(30 + i * w, y, w - 8, 58);
            ctx.fillStyle = disabled ? '#93aa96' : '#f2fae9'; ctx.font = '500 24px system-ui'; ctx.fillText(action, 41 + i * w, y + 14, w - 25);
        });
        return c;
    }
    function hit(ray) {
        if (!pose || !renderer) return null;
        return hitTotemSurface(ray, [{ ...pose, width: hidden ? .20 : .66, height: hidden ? .07 : .66 }]);
    }
    const api = {
        element,
        ...
        attach(gl) {
            renderer?.destroy(); renderer = createSpatialTotemCards(gl, { canvas,
                surfaces: (_position, _right, cards) => pose ? [{ ...pose, width: hidden ? .20 : .66, height: hidden ? .07 : .66, card: cards[0] }] : [] });
        ...
        draw(view) {
            ...
            const card = { id: 'info', hidden, controls: controls(), first: page === 0, last: page === p.length - 1,
                plant: '+INFO · ' + (selection?.plant || 'YOUR COMPANION'), title: help || !selection ? 'Knowledge beside you' : selection.title,
                trail: help ? 'Hold · explore · read' : selection?.breadcrumb || '', lines: p[page], page: (page + 1) + ' / ' + p.length,
                metadata: help || !selection ? 'Hide any time. +INFO restores this panel.' : [selection.scope, selection.status, selection.evidence].filter(Boolean).join(' · ') };
        ...
        activate(ray) {
            ...
            const u = ((target.point.x - pose.center.x) * pose.right.x + (target.point.z - pose.center.z) * pose.right.z) / target.width + .5;
            const v = .5 - (target.point.y - pose.center.y) / target.height;
