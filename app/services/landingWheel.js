...
import {clamp,scrollTurn,gestureIntent,wheelGestureVelocity,decayWheelVelocity,WHEEL_DRAG_RADIANS_PER_PIXEL} from './wheel-model.js';
...
function setMotion(){reduced=media.matches||forcedReduce;hero.dataset.motion=reduced?'reduce':'full';hero.querySelector('#nl-instructions').textContent='Drag in any direction to turn the botanical wheel.';scrollOrigin=scroll;if(reduced){velocity=0;pitchVelocity=0;}requestDraw();}
function reset(){roll=0;yaw=0;pitch=0;velocity=0;pitchVelocity=0;pointerX=0;pointerY=0;scrollOrigin=scroll;status.textContent='Wheel returned to its starting view.';requestDraw();}
...
function draw(now){
 ...
 const live=!reduced;if(live){time+=dt;if(!gesture){roll+=dt*.24;yaw+=velocity*dt;roll+=velocity*dt*.32;velocity=decayWheelVelocity(velocity,dt,false);pitch=clamp(pitch+pitchVelocity*dt,-.85,.85);pitchVelocity=decayWheelVelocity(pitchVelocity,dt,false);}}
 ...
 const targetX=clamp(-.12+pitch+(live?pointerY*.10:0),-.95,.95);
...
listen(host,'pointermove',event=>{
 if(gesture&&event.pointerId===gesture.id){const now=performance.now(),dx=event.clientX-gesture.x,dy=event.clientY-gesture.y;if(gesture.intent==='pending')gesture.intent=gestureIntent(dx,dy,{allowVertical:true});if(gesture.intent==='rotate'){event.preventDefault();if(!host.hasPointerCapture(event.pointerId))host.setPointerCapture(event.pointerId);const stepX=event.clientX-gesture.lastX,stepY=event.clientY-gesture.lastY;yaw+=stepX*WHEEL_DRAG_RADIANS_PER_PIXEL;roll+=stepX*.004;pitch=clamp(pitch+stepY*WHEEL_DRAG_RADIANS_PER_PIXEL*.7,-.85,.85);gesture.lastX=event.clientX;gesture.lastY=event.clientY;gesture.lastAt=now;gesture.samples.push({x:event.clientX,y:event.clientY,at:now});gesture.samples=gesture.samples.filter(sample=>now-sample.at<=140).slice(-8);velocity=wheelGestureVelocity(gesture.samples,gesture.inheritedVelocity,reduced,'x');pitchVelocity=wheelGestureVelocity(gesture.samples,gesture.inheritedPitchVelocity,reduced,'y');requestDraw();}return;}
...
listen(host,'pointerdown',event=>{
 ...
 gesture={lastX:event.clientX,lastY:event.clientY,lastAt:now,id:event.pointerId,x:event.clientX,y:event.clientY,intent:'pending',inheritedVelocity:velocity,inheritedPitchVelocity:pitchVelocity,samples:[{x:event.clientX,y:event.clientY,at:now}]};
});
