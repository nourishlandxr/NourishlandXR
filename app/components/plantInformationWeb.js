...
function detailMarkup(document, state, options) {
    ...
    return `<aside class="pim-web-detail" role="dialog" aria-modal="false" aria-labelledby="pim-web-detail-title-${attribute(node.id)}" data-pim-detail-id="${attribute(node.id)}" data-pim-detail-path="${attribute(node.path)}" style="--pim-category:${attribute(category.color)}">
        <header><div><span>${escapeHtml(category.title)} · ${escapeHtml(evidenceLabel(node.evidenceStatus))}${options.editable ? ` · ${escapeHtml(titleCase(node.status || 'draft'))}` : ''}</span><h2 id="pim-web-detail-title-${attribute(node.id)}">${escapeHtml(node.title)}</h2><nav class="pim-reading-breadcrumbs" aria-label="Topic path">${ancestorsOf(document,node.id).map(ancestor => `<button type="button" data-pim-related-node-id="${attribute(ancestor.id)}">${escapeHtml(ancestor.title)}</button>`).join(' <span aria-hidden="true">›</span> ')}</nav></div><button type="button" data-pim-close-detail aria-label="Close ${attribute(node.title)} details">×</button></header>
