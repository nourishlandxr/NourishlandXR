...
export async function renderFieldGuide(app, encodedProjectId, creator = false) {
    ...
    try {
        ...
        const areaCards = places.map(place => {
            ...
            const icon = creator ? '<i class="field-guide-fireplace-symbol" aria-label="Open Area">🔥</i>' : symbols || '<i class="field-guide-area-symbol" aria-label="Area"></i>';
        ...
        const creationBoard = creator ? `<section class="field-guide-creation-board" aria-labelledby="fieldGuideCreateTitle"><div><p class="welcome-label">ADD TO WEB HUB</p><h2 id="fieldGuideCreateTitle">Create</h2><p>Add a Plant to Home now, or create a named Area for its own spatial layout.</p></div><div class="field-guide-creation-actions"><button type="button" onclick="window.renderLocationFieldMarker('${encoded(guide.project.id)}','plant','without-ar',true)"><strong>+ Plant</strong><span>Add an unassigned Plant to Home.</span></button><button type="button" onclick="window.renderProjectAreaForm('${encoded(guide.project.id)}','field-guide')"><strong>+ Area</strong><span>Create a separate real-world zone.</span></button></div></section>` : '';
        const anchoredCount = places.reduce((sum, place) => sum + place.anchoredCount, 0);
        const markerCount = places.reduce((sum, place) => sum + place.markerCount, 0);
        ...
        const preparationTools = creator ? `<section class="field-guide-preparation" aria-labelledby="fieldGuidePrepareTitle"><div class="field-guide-section-heading"><div><h2 id="fieldGuidePrepareTitle">Prepare this location</h2><p>Connect records to the physical world.</p></div></div><div class="field-guide-preparation-grid"><button type="button" onclick="window.renderLocationMap('${encoded(guide.project.id)}',true,'field-guide')"><strong>Spatial Plan</strong><span>Site Map, Area links and real relative positions.</span></button><button type="button" onclick="window.renderStartingPoints('${encoded(guide.project.id)}')"><strong>Visitor Entrances</strong><span>Create an optional Trail Entrance for guided visitors.</span></button></div><details class="field-guide-anchor-readiness"><summary><span aria-hidden="true">⌖</span><div><strong>${anchoredCount} of ${markerCount} records anchored</strong><p>Open to review or edit every physical-world connection.</p></div></summary><div class="field-guide-anchored-list">${anchoredRows || '<p>No records are anchored yet.</p>'}</div></details></section>` : '';
        ...
        if (creator) {
            app.querySelector('.field-guide-hub')?.insertAdjacentHTML('afterbegin', '<div class="web-context-beacon is-home"><span>WEB HUB LOCATION</span><strong data-web-hub-location>HOME</strong><small>Default workspace for unassigned Plants and new records.</small></div>');
        }
