...
async function loadGuide(projectId) {
    ...
    if (!project) throw new Error('This project is not public.');
...
export async function renderFieldGuideProjects(app) {
    ...
    app.innerHTML = `<div class="screen field-guide"><div class="page-header"><button class="ghost" onclick="window.renderV1Explorer()">Back</button><p class="welcome-label">Field Guide</p><h1>Choose a project</h1><p class="subtitle">Browse plants, maps and information.</p></div><div class="menu-stack">${projects.map(project => `<button class="menu-card" onclick="window.renderFieldGuide('${encoded(project.id)}')"><strong>${escapeHtml(project.name)} Field Guide</strong><span>Project notebook and plant records</span></button>`).join('') || '<div class="panel"><p>No public projects are available.</p></div>'}</div></div>`;
}
...
export async function renderFieldGuide(app, encodedProjectId, creator = false) {
    ...
    try {
        ...
        const backAction = creator ? `window.renderProjectDashboard('${encoded(projectId)}')` : 'window.renderFieldGuideProjects()';
