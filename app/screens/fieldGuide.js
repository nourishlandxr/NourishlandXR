...
export async function renderFieldGuide(app, encodedProjectId, creator = false) {
    ...
    try {
        ...
        const allPlaces = guide.siteGroups.flatMap(group => group.placeGroups.map(placeGroup => ({ ...placeGroup.place, siteName: group.site.name, count: placeGroup.plants.length, markerCount: placeGroup.markerCount, anchoredCount: placeGroup.anchoredCount, hasTotem: placeGroup.hasTotem, hasStartingPoint: placeGroup.hasStartingPoint })));
