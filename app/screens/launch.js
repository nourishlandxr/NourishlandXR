...
function mountLandingSteps(root){
 ...
 const reduced=matchMedia('(prefers-reduced-motion: reduce)');let index=0,timer=0,disposed=false;
 ...
 const schedule=()=>{clearTimeout(timer);if(disposed)return;timer=setTimeout(()=>{show(index+1);schedule();},6000);};
 show(0);schedule();
