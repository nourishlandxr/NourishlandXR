...
export async function renderAnalogPlace(target = app, placeId) {
    ...
    app.innerHTML = `<div class="screen analog-explorer analog-print-page"><header class="page-header analog-header"><button class="ghost analog-navigation" onclick="window.renderAnalogExplorer()">Back</button><p class="print-kicker">HILLYARDS FOOD FOREST</p><h1>hillyards loaded</h1><p class="subtitle">Area: ${escapeHtml(placeId)}</p></header><div class="analog-plant-list">${plants.length ? plants.map((plant, index) => plantRow(plant, index)).join('') : '<p>No plants are listed for this Area.</p>'}</div><div class="analog-qr-space">QR CODE</div></div>`;
}
...
export async function renderAnalogPlant(target = app, encodedInstanceId) {
    ...
    app.innerHTML = `<div class="screen analog-explorer analog-profile analog-print-page"><header class="page-header analog-header"><button class="ghost analog-navigation" onclick="window.renderAnalogPlace('${escapeHtml(plant.placeId)}')">Back</button><p class="print-kicker">HILLYARDS FOOD FOREST · ${escapeHtml(plant.placeId)}</p><h1>${escapeHtml(plant.commonName)}</h1><p class="subtitle"><em>${escapeHtml(plant.scientificName)}</em></p></header><dl class="analog-profile-grid">${fields.map(([label, value]) => `<div><dt>${label}</dt><dd>${escapeHtml(value || 'Not entered.')}</dd></div>`).join('')}</dl><div class="analog-qr-space">QR CODE</div></div>`;
}
...
export async function renderAnalogLibraryPlant(target = app, encodedPlantId) {
    ...
    app.innerHTML = `<div class="screen analog-explorer analog-profile analog-print-page"><header class="page-header analog-header"><button class="ghost analog-navigation" onclick="window.renderGlobalPlantList()">Back</button><p class="print-kicker">NOURISHLAND PLANT LIBRARY</p><h1>${escapeHtml(plant.commonName || 'Unnamed plant')}</h1><p class="subtitle"><em>${escapeHtml(plant.scientificName || '')}</em></p></header><dl class="analog-profile-grid">${fields.map(([label, value]) => `<div><dt>${label}</dt><dd>${escapeHtml(value || 'Not entered.')}</dd></div>`).join('')}</dl><div class="analog-qr-space">QR CODE</div></div>`;
}
