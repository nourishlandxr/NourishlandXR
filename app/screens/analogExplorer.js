...
export async function renderAnalogPlace(target = app, placeId) {
    ...
    app.innerHTML = `<div class="screen analog-explorer analog-print-page"><header class="page-header analog-header"><button class="ghost analog-navigation" onclick="window.renderAnalogExplorer()">Back</button><p class="print-kicker">HILLYARDS FOOD FOREST</p><h1>Main Food Forest</h1><p class="subtitle">Area: ${escapeHtml(placeId)}</p></header><div class="analog-plant-list">${plants.length ? plants.map((plant, index) => plantRow(plant, index)).join('') : '<p>No plants are listed for this Area.</p>'}</div><div class="analog-qr-space">QR CODE</div><div class="analog-print-footer">${printButton()}</div></div>`;
}
