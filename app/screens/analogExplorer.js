...
export async function renderAnalogPlant(target = app, encodedInstanceId) {
    ...
    app.innerHTML = `<div class="screen analog-explorer analog-profile analog-print-page"><header class="page-header analog-header"><button class="ghost analog-navigation" onclick="window.renderAnalogPlace('${escapeHtml(plant.placeId)}')">Back</button><p class="print-kicker">HILLYARDS FOOD FOREST Â· ${escapeHtml(plant.placeId)}</p><h1>${escapeHtml(plant.commonName)}</h1><p class="subtitle"><em>${escapeHtml(plant.scientificName)}</em></p></header><dl class="analog-profile-grid">${fields.map(([label, value]) => `<div><dt>${label}</dt><dd>${escapeHtml(value || 'Not entered.')}</dd></div>`).join('')}</dl><div class="analog-qr-space">QR CODE</div></div>`;
}
