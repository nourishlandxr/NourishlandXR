...
export async function renderXrProjects(app) {
    ...
    disableTargetReticle();
    try {
        const projects = (await loadProjects(true)).filter(project => project.id !== 'plant-library');
        app.innerHTML = `<div class="screen explorer-entry"><div class="page-header"><button class="ghost" onclick="window.renderV1Explorer()">Back</button><p class="welcome-label">XR Explorer</p><h1>Choose a project</h1><p class="subtitle">Immersive, location-based learning.</p></div><div class="menu-stack">${projects.map(project => `<button class="menu-card" onclick="${project.id === 'Hillyards' ? 'window.openHillyardsExplorer()' : `window.renderExplorerSites(${JSON.stringify(project)})`}"><strong>${project.name}</strong><span>Open XR project</span></button>`).join('') || '<div class="panel"><p>No public projects are available.</p></div>'}</div></div>`;
    } catch (error) { app.innerHTML = errorScreen(`Project data could not be loaded: ${error.message}`); }
}
