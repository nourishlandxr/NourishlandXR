...
function projectBreadcrumbMarkup(project, area, currentLabel = '') {
    ...
    const areaAction = `window.renderProjectAreaDashboard('${encoded(project.id)}','${encoded(area.id)}')`;
    const areaSegment = isDefaultHomeArea(area) ? `<strong>HOME</strong>` : `<button type="button" onclick="${areaAction}">${escapeHtml(areaLabel)}</button>`;
...
export async function renderProjectAreaDashboard(app, encodedProjectId, encodedAreaId, options = {}) {
    ...
    try {
        ...
        const unplacedTotemRow = checkpoint ? '' : `<article class="area-content-entry area-content-card is-totem-entry" role="button" tabindex="0" onclick="window.renderAreaCheckpointForm('${encoded(context.project.id)}', '${encoded(context.area.id)}')"><span class="latest-entry-icon" aria-hidden="true">${markerIcon('area_checkpoint')}</span><span class="latest-entry-copy"><strong>${escapeHtml(context.area.name)} Totem Marker</strong><span>Totem Marker · Not yet placed</span></span><span class="area-entry-actions"><button class="global-ar-action spatial-focus-button compact-ar-action" type="button" onclick="event.stopPropagation();window.openProjectAreaAr('${encoded(context.project.id)}', '${encoded(context.area.id)}', '', 'area_checkpoint')">PLACE IN AR</button></span></article>`;
