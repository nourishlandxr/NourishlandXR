...
function dashboardGuidance(projectId, { hasArea, startingConfigured, freshProject, nonPlantMode = false }) {
    ...
    const content = {
        ...
        arMode: {
            title: 'Use project information in AR',
            full: nonPlantMode
                ? 'AR is an access mode for placing and opening the same project Content beside real objects. Return to Overview, Map or Content whenever you want to organise and deepen those records.'
                : 'AR is an access mode for placing and opening the same Plants, Notes and knowledge held in the project dashboard. Return to Overview, Map or Content whenever you want to organise and deepen those records.',
            short: 'AR gives project information a place while the dashboard keeps it organised.',
            actionLabel: 'Create your first Marker',
            action: `window.openCreatorArMode('${encoded(projectId)}')`
        },
        helpGuide: {
            title: 'Suggest reading: Project Guide',
            full: 'The Project Guide explains the main workflows, Areas, Totems, Plant Live Tags and how AR provides spatial access to dashboard Content.',
    ...
    try {
        ...
        app.innerHTML = `<div class="screen content-mode-screen">
            ...
            <nav class="content-mode-tool-grid" aria-label="Content Mode tools">
                <button type="button" onclick="window.renderFieldGuide('${encoded(project.id)}', true)"><strong>Content</strong><span>Manage Home, Plants, Areas, Totems and their information.</span></button>
...
export async function renderBrowseContent(app, encodedProjectId, creator = false) {
    ...
    const back = creator ? `window.renderProjectDashboard('${encoded(project.id)}')` : `window.renderVisitorLocationIntro('${encoded(project.id)}')`;
        app.innerHTML = `<div class="screen browse-content-screen"><div class="page-header"><button class="ghost" onclick="${back}">Back</button><h1>Browse Content</h1><p class="subtitle">Access the project’s plants, stories, checkpoints and maps without entering AR.</p></div><div class="content-type-list"><button class="content-type-row" type="button" onclick="window.renderFieldGuide('${encoded(project.id)}', ${creator})"><strong>${creator ? 'Content' : 'Field Guide'}</strong><span>Browse plants and visitor-visible information.</span></button><button class="content-type-row" type="button" onclick="window.renderLocationMap('${encoded(project.id)}', ${creator})"><strong>Map</strong><span>View content by location without using the camera.</span></button></div>${rows ? `<section class="latest-entries-section"><h2>Stories and checkpoints</h2><div class="latest-entry-list">${rows}</div></section>` : ''}</div>`;
}
...
export async function renderLocationMap(app, encodedProjectId, creator = true, returnContext = '') {
    ...
    try {
        ...
        const dashboardMapHeader = creator && returnContext !== 'print-center'
...
function plantProfileEditorMarkup(entry, profile, physicalAnchorMarkup = '', projectId = '', siteId = '', returnToAr = false, returnContext = '') {
    ...
    const previewAction = `window.startPlantEditorPreview('${encoded(projectId)}','${encoded(siteId)}','${encoded(entry.place?.id || '')}','${encoded(entry.marker.id)}')`;
    return `<section class="plant-encyclopedia-card">
        ...
        <section class="plant-qr-anchor-card plant-virtual-tag-card"><span aria-hidden="true">▦</span><div><div class="plant-live-tag-heading"><strong>PLANT LIVE TAG</strong><button type="button" class="plant-profile-info-bubble" data-info-trigger data-info-source="plantLiveTagHelp" aria-expanded="false" aria-controls="plantLiveTagHelp" aria-label="About Plant Live Tags" onclick="window.toggleInfoOverlay(this)">i</button></div><p id="plantLiveTagHelp" data-info-title="Plant Live Tag" hidden>Prepare this Plant profile to become a scannable garden tag that opens its Content profile.</p><label class="ar-inline-checkbox" for="projectEntryVirtualTag"><input id="projectEntryVirtualTag" type="checkbox" ${profile.virtual_tag_enabled === true ? 'checked' : ''} /> <span>Make this Plant a Plant Live Tag</span></label>${physicalAnchorMarkup}</div></section>
        </div>
        <div class="plant-profile-pim-cta"><div><strong>Plant Information Mesh</strong><small>Open the expandable plant knowledge workspace shared by Web Mode and AR.</small></div><div class="plant-profile-pim-actions"><button type="button" class="plant-profile-pim-open" onclick="window.openProjectPim('${encoded(projectId)}','${encoded(entry.marker.id)}',${Boolean(returnToAr)},'${encoded(returnContext)}')">Open Info Mesh</button><button type="button" class="plant-profile-pim-preview" onclick="${previewAction}" aria-label="Preview ${escapeAttribute(profile.common_name || entry.marker.name)} in AR">Preview in AR</button></div></div>
    </section>`;
}
...
function plantProfileHeaderMarkup(project, entry, placement, profile) {
    ...
    return `<header class="plant-profile-header">
        <div class="plant-profile-heading">
            <p class="plant-profile-kicker">PLANT EDITOR <span aria-hidden="true">·</span> ${escapeHtml(areaLabel)}</p>
...
export async function openProjectEntry(app, encodedProjectId, encodedMarkerId, returnToAr = false, returnContext = '', pimInitialState = null) {
    ...
    if (!plant && (returnContext === 'field-guide' || returnContext === 'webhub')) {
        ...
        if (backButton) {
            backButton.textContent = 'Back to Content';
